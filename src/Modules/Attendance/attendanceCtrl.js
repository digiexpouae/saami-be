...
class AttendanceController {
  ...
  async getEmployeeAttendanceRecords(req, res) {
    try {
      ...
      const records = await AttendanceService.getEmployeeAttendanceRecords(
        employeeId
      );
  ...
  async getWarehouseEmployeesStatus(req, res) {
    try {
      ...
      const records = await AttendanceService.getWarehouseEmployeesStatus(
        warehouseId
      );
      return handleResponse(res, 200, "Attendance record retrieved", records);
    } catch (error) {
  ...
  async getCheckinStatus(req, res) {
    ...
    async getAllEmployeeAttendances(req, res) {
        try {
          const bodyDto = req.body;
        const records = await AttendanceService.getAllEmployeeAttendances(
          bodyDto
        );
        return handleResponse(res, 200, "All attendance retrieved", records);
      } catch (error) {
            return handleError(res, 400, error.message);
